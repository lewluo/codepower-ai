...
export class WebSocketHandler {
    ...
    handleTextMessage(message) {
        if (message.type === 'hello') {
            ...
            log(`提案状态更新: ${JSON.stringify(message)}`, 'info');
            if (this.onProposalStateChange) {
                this.onProposalStateChange(message);
            }
    ...
    sendActionMessage(actionType, payload = {}) {
        ...
        try {
            ...
            log(`发送控制消息: ${JSON.stringify(message)}`, 'info');
            return true;
        } catch (error) {
            log(`发送控制消息失败: ${error.message}`, 'error');
            return false;
        }
    }

    sendAccept() {
        return this.sendActionMessage('accept');
    }

    sendReject() {
        return this.sendActionMessage('reject');
    }
