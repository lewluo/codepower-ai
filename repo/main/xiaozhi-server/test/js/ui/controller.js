...
class UIController {
    constructor() {
        ...
        this.handleProposalState = this.handleProposalState.bind(this);
    }
    ...
    initEventListeners() {
        ...
        if (acceptBtn) {
            acceptBtn.addEventListener('click', () => {
                const wsHandler = getWebSocketHandler();
                if (!wsHandler.sendAccept()) {
                    this.addChatMessage('当前未连接，无法发送接受指令', false);
                }
            });
        }
        ...
        if (rejectBtn) {
            rejectBtn.addEventListener('click', () => {
                const wsHandler = getWebSocketHandler();
                if (!wsHandler.sendReject()) {
                    this.addChatMessage('当前未连接，无法发送拒绝指令', false);
                }
            });
        }
    ...
    updateDialButton(isConnected) {
        ...
        [acceptBtn, rejectBtn].forEach(btn => {
            ...
            btn.title = isConnected ? '发送确认指令' : '请先连接服务器';
        });

        if (acceptBtn) {
            acceptBtn.classList.toggle('proposal-pending', isConnected && this.proposalPending);
            acceptBtn.title = isConnected
                ? (this.proposalPending ? '当前有待确认提案，点击采纳' : '当前没有待确认提案')
                : '请先连接服务器';
        }

        if (rejectBtn) {
            rejectBtn.classList.toggle('proposal-pending', isConnected && this.proposalPending);
            rejectBtn.title = isConnected
                ? (this.proposalPending ? '当前有待确认提案，点击拒绝' : '当前没有待确认提案')
                : '请先连接服务器';
        }
    }
    ...
    // Add chat message
    addChatMessage(content, isUser = false, extraClass = '') {
        const chatStream = document.getElementById('chatStream');
        if (!chatStream) return;

        const messageDiv = document.createElement('div');
        messageDiv.className = `chat-message ${isUser ? 'user' : 'ai'}`;
        messageDiv.innerHTML = `<div class="message-bubble ${extraClass}">${content}</div>`;
    ...
    handleProposalState(message) {
        const proposalKey = `${message.function_name || ''}:${message.task || ''}:${message.proposal_text || ''}`;

        if (message.state === 'pending') {
            this.proposalPending = true;
            if (proposalKey !== this.lastProposalKey) {
                this.addChatMessage(`待确认：${message.proposal_text || '当前有一个待确认操作。'}`, false, 'proposal-card');
                this.lastProposalKey = proposalKey;
            }
        } else {
            this.proposalPending = false;
            this.lastProposalKey = null;
            if (message.state === 'accepted') {
                this.addChatMessage('已采纳，开始执行。', false);
            } else if (message.state === 'rejected') {
                this.addChatMessage('已拒绝本次执行。', false);
            } else if (message.state === 'completed') {
                this.addChatMessage('本次提案执行完成。', false);
            }
        }

        this.updateDialButton(getWebSocketHandler().isConnected());
    }
    ...
    async handleConnect() {
        ...
        try {
            ...
            wsHandler.onProposalStateChange = (message) => {
                this.handleProposalState(message);
            };
