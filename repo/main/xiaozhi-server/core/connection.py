...
from core.handle.proposalHandler import stage_pending_proposal, is_decision_tool
    ...
    def chat(self, query, depth=0):
        ...
        if tool_call_flag:
            ...
            if not bHasError and len(tool_calls_list) > 0:
                ...
                # 收集所有工具调用的 Future
                futures_with_data = []
                decision_tool_call = next(
                    (
                        tool_call_data
                        for tool_call_data in tool_calls_list
                        if is_decision_tool(tool_call_data["name"])
                    ),
                    None,
                )
                if decision_tool_call is not None:
                    self.logger.bind(tag=TAG).info(
                        f"检测到需确认的决策型工具调用: {decision_tool_call['name']}"
                    )
                    asyncio.run_coroutine_threadsafe(
                        stage_pending_proposal(
                            self,
                            decision_tool_call,
                            query or "请执行刚才的操作",
                        ),
