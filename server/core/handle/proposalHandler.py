...
def _execute_tool_with_existing_pipeline(
    ...
    # 长任务先给用户一句语音反馈（不用 _speak_text，避免嵌套 FIRST/LAST 冲突）
    display = _display_tool_name(function_name)
    feedback = f"好的，{display}开始执行，完成后告诉你结果。"
    conn.tts.tts_one_sentence(conn, ContentType.TEXT, content_detail=feedback)
    conn.tts.store_tts_text(conn.sentence_id, feedback)
    conn.dialogue.put(Message(role="assistant", content=feedback))
