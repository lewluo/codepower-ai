...
LvglImage* CreateSmileLogoImage() {
    constexpr int kWidth = 96;
    constexpr int kHeight = 96;
    constexpr uint16_t kWhite = 0xFFFF;

    auto* data = static_cast<uint16_t*>(heap_caps_malloc(
        kWidth * kHeight * sizeof(uint16_t), MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT));
    if (data == nullptr) {
        ESP_LOGW(TAG, "Failed to allocate smile logo image, falling back to built-in emoji");
        return new LvglSourceImage(&emoji_1f642_64);
    }

    std::memset(data, 0, kWidth * kHeight * sizeof(uint16_t));

    auto fill_rect = [data](int x0, int y0, int x1, int y1, uint16_t color) {
        for (int y = y0; y < y1; ++y) {
            for (int x = x0; x < x1; ++x) {
                data[y * 96 + x] = color;
            }
    ...
    // Left eye
    fill_rect(20, 24, 28, 40, kWhite);
    fill_rect(16, 28, 32, 36, kWhite);
    fill_rect(20, 20, 28, 24, kWhite);
    fill_rect(20, 40, 28, 44, kWhite);

    // Right eye
    fill_rect(68, 24, 76, 40, kWhite);
    fill_rect(64, 28, 80, 36, kWhite);
    fill_rect(68, 20, 76, 24, kWhite);
    fill_rect(68, 40, 76, 44, kWhite);

    // Mouth
    fill_rect(26, 62, 30, 74, kWhite);
    fill_rect(30, 68, 38, 78, kWhite);
    fill_rect(38, 72, 58, 84, kWhite);
    fill_rect(58, 68, 66, 78, kWhite);
    fill_rect(66, 62, 70, 74, kWhite);

    return new LvglAllocatedImage(
        data,
        kWidth * kHeight * sizeof(uint16_t),
        kWidth,
        kHeight,
        kWidth * static_cast<int>(sizeof(uint16_t)),
